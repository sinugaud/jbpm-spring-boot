...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "jbpm-spring-boot-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run  -p 8090:8090 --name jbpm-spring-boot-service sinugaud/jbpm-spring-boot-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app jbpm-spring-boot-service:1.0-SNAPSHOT
		call oc expose svc/jbpm-spring-boot-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd jbpm-spring-boot-service
		cd target
