...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "jbpm-spring-boot-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name jbpm-spring-boot-service apps/jbpm-spring-boot-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app jbpm-spring-boot-service:1.0-SNAPSHOT
    oc expose svc/jbpm-spring-boot-service
else

	echo "Launching the application locally..."
	pattern="jbpm-spring-boot-service"
	files=( $pattern )
	cd ${files[0]}
